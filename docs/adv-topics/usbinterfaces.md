...
!!! warning "Moving USB Ports"
    If you move a USB audio interface between USB ports on your server, the device string (`devstr=`) **will** probably change, and this may very likely *break* your configuration (Asterisk will no longer be able to find the device). You can use `asl-find-sound` to find the new device string, and update your relevant `.conf` file manually, or open the `.conf` file, delete the existing value and leave `devstr=` blank to have it re-discovered.
