...
`node_<node>`|telemetry stanza key|Send this courtesy tone when the matching node un-keys.  If there is no match, `linkunkeyct` is used.  If `linkunkeyct` is not defined, then no courtesy tone will be sent when a networked user un-keys. 
...
node_4321 = ct8                     ; Send this courtesy tone if node 4321 has unkeyed. linkunkeyct must be defined for this to work.
