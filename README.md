...
Alternately, install Zensical (see below per platform),
...
## Full Local Editing
Installing [Zensical](https://zensical.org/docs/get-started/#installation) is required to view/test
...
Browse http://localhost:8000 to see the existing documentation set.
As you edit files in the `docs/` directory, the web browser window will be
updated in real time by the running `zensical serve` command. Watch that
...
## Setup Zensical
ASL-Manual is generated with [Zensical](https://zensical.org/). While it is possible
to contribute to the manual without installing Zensical, it would be difficult.
Fortunately Zensical is available on all normal user platforms. In addition to Zensical, Python 3 and Git is needed.

### Windows 11

#### Python
Python is not natively installed with Windows like it is with Linux. Installation is very straight forward:
- Open a PowerShell terminal
- Install the Python Install Manager using `winget install 9NQ7512CXL7T`. (Note: 9NQ7512CXL7T is the package unique ID for Python Installer.)
- Install Python 3.14 using the `py` command - `py install 3.14`

Overall, the installation is:
...
- Install all requirements from PowerShell
    ```bash
...
- Create a virtual python environment and install the necessary packages
    ```bash
    python3 -m venv ~/.zensical
    . ~/.zensical/bin/activate
    pip install -r requirements.txt
    ```

Every new shell, enter the python venv with `. ~/.zensical/bin/activate` before
beginning to use any `zensical` commands.
...
This procedure will configure a WSL2 environment on your local machine to run Linux under (for those more familiar with operating Git in CLI under Linux), and integrate that with using VS Code in the host Windows environment for editing the manual. Unless you are a Windows power user and adept at Linux, this method is not recommended.
